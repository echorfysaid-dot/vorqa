...
export async function GET(request: Request) {
  ...
  if (isApiError(user)) return NextResponse.json({ error: user.error }, { status: user.status });
  ...
  if (isApiError(rows)) return NextResponse.json({ error: rows.error }, { status: rows.status });
