...
export async function POST(request: Request) {
  ...
  if (parseMode((body as { mode?: unknown }).mode) === "vora_intelligence") {
    ...
    const projectId = parseProjectId((body as { projectId?: unknown }).projectId);
    const analysisSessionId = typeof (body as { sessionId?: unknown }).sessionId === "string" ? sanitizeAiText((body as { sessionId: string }).sessionId, 120) : undefined;
    ...
    if (taskIntent === "contract_review") {
      ...
      return NextResponse.json(await attachProjectPersistence(result, { projectId, ownerId: user.id, token, sessionId: analysisSessionId, toolType: "contract_review" }), { status: result.status === "success" ? 200 : 400 });
    }
    ...
    if (taskIntent === "cost_review" && (body as { boqDocument?: unknown }).boqDocument) {
      ...
      return NextResponse.json(await attachProjectPersistence(result, { projectId, ownerId: user.id, token, sessionId: analysisSessionId, toolType: "boq_review" }), { status: result.status === "success" ? 200 : 400 });
    }
    ...
    if (taskIntent === "risk_assessment") {
      ...
      return NextResponse.json(await attachProjectPersistence(result, { projectId, ownerId: user.id, token, sessionId: analysisSessionId, toolType: "risk_assessment" }), { status: result.status === "success" ? 200 : 400 });
    }
    ...
    if (taskIntent === "planning_review") {
      ...
      return NextResponse.json(await attachProjectPersistence(result, { projectId, ownerId: user.id, token, sessionId: analysisSessionId, toolType: "planning_review" }), { status: result.status === "success" ? 200 : 400 });
    }
    ...
    if (taskIntent === "site_report_review") {
      ...
      return NextResponse.json(await attachProjectPersistence(result, { projectId, ownerId: user.id, token, sessionId: analysisSessionId, toolType: "site_report_review" }), { status: result.status === "success" ? 200 : 400 });
    }
    ...
    if (taskIntent === "executive_summary") {
      ...
      return NextResponse.json(await attachProjectPersistence(result, { projectId, ownerId: user.id, token, sessionId: analysisSessionId, toolType: "executive_summary" }), { status: result.status === "success" ? 200 : 400 });
    }
